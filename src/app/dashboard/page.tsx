...
import { useAppContext, AppUser } from "@/context/app-context";
...
// Função reutilizável para calcular o saldo efetivo, removendo duplicidade de código
const calculateEffectiveBalance = (user: AppUser | null): number => {
  if (!user) return 0;
  let balance = user.totalPoints ?? 0;
  const quota = user.distributableQuota;

  // Se a cota de distribuição expirou, o saldo restante da cota é "perdido" do total
...
export default function DashboardPage() {
  ...
  // Preenche o modal de edição quando um usuário é selecionado
  useEffect(() => {
    if (editModalUser) {
  ...
  // Calcula o saldo do usuário logado usando a função reutilizável
  const effectiveBalance = useMemo(() => calculateEffectiveBalance(currentUser), [currentUser]);

  // Função para conceder pontos (ação de admin)
  ...
  // Função para transferir pontos (ação de colaborador)
  ...
  // Função para atualizar dados do usuário (ação de admin)
  ...
  return (
    <div className="container mx-auto p-4 md:p-8">
      {/* Cabeçalho com nome e menu do usuário */}
      <header className="mb-8 flex items-center justify-between">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent className="w-56" align="end" forceMount>
            ...
            <DropdownMenuSeparator />
            <DropdownMenuItem asChild><Link href="/profile">Meu Perfil</Link></DropdownMenuItem>
      ...
      {/* Alerta de Meta de Distribuição */}
      {!isAdmin && currentUser.distributableQuota?.remaining > 0 && (
        <Card className="mb-6 border-blue-500 bg-blue-50">
          <CardContent className="p-4">
            ...
            {currentUser.distributableQuota.expiresAt && (
              <p className="mt-1 text-sm text-blue-600">Prazo final: <strong>{new Date(currentUser.distributableQuota.expiresAt).toLocaleDateString('pt-BR')}</strong>.</p>
            )}
          </CardContent>
        </Card>
      )}

      {/* Grid principal com Saldo, Ações e Histórico */}
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-5">
        <div className="flex flex-col gap-6 lg:col-span-2">
          ...
          {/* Botões de Ação (Adicionar ou Transferir Pontos) */}
          {isAdmin ? (
            <Dialog open={isAddPointsDialogOpen} onOpenChange={setIsAddPointsDialogOpen}>
              ...
              <DialogContent className="sm:max-w-md">
                ...
                <div className="grid gap-4 py-4">
                  <Select onValueChange={setSelectedUserForAction}>
                  ...
                  <div className="flex items-center space-x-2">
                    <Checkbox id="is-quota" checked={isQuota} onCheckedChange={(checked: boolean) => setIsQuota(checked)} />
                    <Label htmlFor="is-quota">Meta de distribuição obrigatória</Label>
                  </div>
                </div>
                <DialogFooter>
                  <Button onClick={handleGrantPoints} disabled={isLoadingAction}>
                    {isLoadingAction ? "Adicionando..." : "Adicionar Pontos"}
                  </Button>
                </DialogFooter>
              </DialogContent>
            </Dialog>
            ...
            <Dialog open={isTransferDialogOpen} onOpenChange={setIsTransferDialogOpen}>
              ...
              <DialogContent className="sm:max-w-md">
                ...
                <DialogFooter>
                  <Button onClick={handleTransferPoints} disabled={isLoadingAction}>
                    {isLoadingAction ? "Transferindo..." : "Transferir"}
                  </Button>
                </DialogFooter>
              </DialogContent>
            </Dialog>
        ...
        {/* Tabela de Histórico Pessoal */}
        <div className="lg:col-span-3">
          <Card>
            <CardHeader>
              <CardTitle>Meu Histórico de Transações</CardTitle>
            </CardHeader>
            <CardContent>
              <Table>
                <TableHeader>
                ...
                <TableBody>
                  {pointsHistory.filter(tx => tx.userId === currentUser.id).length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={2} className="h-24 text-center text-muted-foreground">Nenhuma transação no seu histórico.</TableCell>
                    </TableRow>
                  ) : (
                    pointsHistory
                      ...
                      .map(tx => (
                        <TableRow key={tx.id}>
                          <TableCell>
                            ...
                            <p className="text-sm text-muted-foreground">{new Date(tx.date).toLocaleString("pt-BR")}</p>
                          </TableCell>
      ...
      {isAdmin && (
        <>
          {/* Modal de Histórico de Colaborador (Conteúdo a ser implementado) */}
          <Dialog open={!!historyModalUser} onOpenChange={(isOpen) => !isOpen && setHistoryModalUser(null)}>
            <DialogContent className="sm:max-w-6xl">
              <DialogHeader>
                <DialogTitle>Histórico de {historyModalUser?.name}</DialogTitle>
              </DialogHeader>
              {/* TODO: Implementar a tabela de histórico do colaborador selecionado */}
              <p className="py-4">O histórico do colaborador aparecerá aqui.</p>
            </DialogContent>
          </Dialog>
            ...
            <DialogContent className="sm:max-w-md">
              ...
              <div className="grid gap-4 py-4">
                ...
                <div className="grid gap-2">
                  ...
                  <Select
                    value={editedRole}
                    onValueChange={(value: "administrador" | "colaborador") => setEditedRole(value)}
                  >
          ...
          {/* Tabela de Visão Geral dos Colaboradores */}
          <Card className="mt-8">
            <CardHeader>
              <CardTitle>Visão Geral dos Colaboradores</CardTitle>
            </CardHeader>
            <CardContent>
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead className="w-[400px]">Colaborador</TableHead>
                  ...
                  {users.filter(u => u.id !== currentUser.id).map(user => (
                    <TableRow key={user.id}>
                      ...
                      <TableCell className="text-right font-mono">
                        {calculateEffectiveBalance(user).toLocaleString("pt-BR")}
                      </TableCell>
